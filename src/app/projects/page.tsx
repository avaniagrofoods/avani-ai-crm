...
const preconfiguredProjects = [
  { id: "p1", name: "Latur Personal Loan Drive 2026", status: "ACTIVE", target: "₹2.5 Cr", createdAt: new Date().toISOString() },
  { id: "p2", name: "Business Expansion & Machinery OD/CC", status: "ACTIVE", target: "₹5.0 Cr", createdAt: new Date().toISOString() },
  { id: "p3", name: "Doctor & CA Professional Campaign", status: "ACTIVE", target: "₹1.5 Cr", createdAt: new Date().toISOString() },
  { id: "p4", name: "Global Education Loan Admissions Intake", status: "ACTIVE", target: "₹3.0 Cr", createdAt: new Date().toISOString() }
];

export default function ProjectsPage() {
  const [items, setItems] = useState<any[]>(preconfiguredProjects);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-zinc-900 border border-zinc-800 p-6 rounded-2xl shadow-md">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-white flex items-center gap-3">
            ...
            Loan Projects & Portfolio Campaigns
          </h2>
          <p className="text-sm text-zinc-400 mt-1">Manage target loan portfolios, sales campaigns, and bank tie-ups.</p>
        </div>
        <button 
        ...
        <table className="w-full text-sm text-left text-zinc-400">
          <thead className="text-xs text-zinc-300 uppercase bg-zinc-800/50 border-b border-zinc-800">
            <tr>
              ...
              <th className="px-6 py-4 font-semibold">Target Target Portfolio</th>
          ...
          <tbody className="divide-y divide-zinc-800/80">
            {items.map((item) => (
              <tr key={item.id} className="hover:bg-zinc-800/30 transition-colors">
                ...
                <td className="px-6 py-4 text-xs font-mono text-emerald-400">{item.target}</td>
            ...
            <form onSubmit={handleAddProject} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  placeholder="e.g. Housing Loan Mela Q3"
              ...
              <div>
                <label className="text-xs font-medium text-zinc-400 block mb-1">Target Portfolio (Amount)</label>
                <input 
                  type="text" 
                  value={target}
